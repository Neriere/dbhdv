...
def load_or_download_items_db(force=False):
    ...
    if not need_download:
        try:
            ...
            # Si la base local es antigua y le faltan runas avanzadas o ingredientes suplementarios, re-descargar
    ...
    try:
        ...
        if r.status_code == 200:
            downloaded_dict = r.json()
            ITEMS_DB.update(downloaded_dict)
            ITEMS_DB.update(DEFAULT_RUNES_DB)
...
def get_item_name(item_id):
    ...
    # Auto-resolución en vivo desde DofusDB para cualquier objeto desconocido
    try:
