...
export async function initDB() {
  ...
  initDbPromise = (async () => {
    try {
      ...
      // Limpiar precios corruptos / TypeID de categorías (ej: Esquíritu majestuoso #31521 = 6k, Pergaminos = 4k, o equipables 985, 125, 211 = 6k)
      try {
...
export async function bulkSaveProfileCoefficients(
  ...
  // Query existing coefficients in DB to accurately calculate updated vs skipped
  ...
  const profileCoeffs = await getProfileCoefficients(pid);
  return {
    ...profileCoeffs,
    updatedCount,
    skippedCount,
...
export async function processAndIngestMarketPrice(
  ...
  // 1. Resolver el objeto real desde SQLite / catálogo / DofusDB
  ...
  // Si no existe en la base de datos de Dofus, o es una colisión corrupta (ej: Puré pic-feil para ID!=35089)
