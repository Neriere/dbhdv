...
export function isOmittedItem(item: {
  ...
  typeId?: number;
  type?: { id?: number; superCategoryId?: number; name?: { es?: string; fr?: string; en?: string } | string };
}): boolean {
  if (!item) return true;

  const typeId = Number(item.typeId || item.type?.id || 0);

  // 1. Smithmagic / Runes / non-trade utility stones
  if ([18, 31, 74, 78, 85, 97, 189, 211, 233, 258, 307, 308].includes(typeId)) {
    return true;
  }

  // 2. Maps & Map Fragments type IDs (174: Mapa, 175: Fragmento de mapa)
  if ([174, 175].includes(typeId)) {
    return true;
  }

  // 3. Cosmetics, Appearance, Roleplay, Roleplay Buffs, Titles, Emotes, Quests type IDs
  if ([113, 166, 173, 188, 199, 200, 203, 204, 214, 222, 246, 247, 248, 249, 250, 251, 252, 304, 324].includes(typeId)) {
    return true;
  }

  // 4. Quests and unused non-craftable item types (Ensure valid resource types like 128, 150, 153 are NOT omitted)
  if ([80, 126, 127, 129, 131, 132, 133, 136, 137, 139, 140, 141, 142, 143, 146, 147, 148, 149, 155, 156, 168, 171, 178, 312].includes(typeId)) {
    return true;
  }

  const nameStr = typeof item.name === 'string' ? item.name : '';
  const nameEs = (typeof item.name === 'object' ? item.name?.es || '' : nameStr).toLowerCase();
  ...
  const typeNameEs = (typeof item.type?.name === 'object' ? item.type.name?.es || '' : typeNameStr).toLowerCase();
  ...
  // Elemental stones check ("piedra de fuego", "piedra de tierra", "piedra de agua", "piedra de aire")
  if (
    text.includes('piedra de fuego') || text.includes('pierre de feu') || text.includes('fire stone') ||
    text.includes('piedra de tierra') || text.includes('pierre de terre') || text.includes('earth stone') ||
    text.includes('piedra de agua') || text.includes('pierre d\'eau') || text.includes('water stone') ||
    text.includes('piedra de aire') || text.includes('pierre d\'air') || text.includes('air stone')
  ) {
    return true;
  }
  ...
  if (
    text.includes('fragmento de mapa') || text.includes('fragment de carte') || text.includes('map fragment') ||
    text.includes('mapa de') || text.includes('carte de trésor') || text.includes('carte au trésor')
  ) {
    return true;
  }
  ...
  if (
    text.includes('runa de') || text.includes('runa ') ||
    text.includes('rune de') || text.includes('rune ') ||
    text.includes('forjamagia') || text.includes('forgemagie') ||
    text.includes('transcendencia') || text.includes('trascendencia') ||
    text.includes('orbe de') || text.includes('grabado de') ||
...
export function getJobForItem(
  ...
  recipe?: { jobId?: number; job?: { id?: number; name?: any }; resultId?: number } | null
...
export function isCosmeticItem(item: {
  ...
  if (item.type?.superCategoryId && COSMETIC_SUPER_CATEGORY_IDS.includes(item.type.superCategoryId)) {
    return true;
  }
  ...
  for (const kw of COSMETIC_KEYWORDS) {
    if (nameEs.includes(kw) || nameFr.includes(kw) || typeNameEs.includes(kw) || typeNameFr.includes(kw)) {
      return true;
    }
  }
