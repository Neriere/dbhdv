...
export function generateBackupData(): DbhdvBackupData {
  ...
  if (typeof window !== 'undefined') {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (
        key.startsWith('dofus_crushing_coeff_') ||
        key.startsWith('dofus_user_item_coefficients') ||
        key.startsWith('dofus_user_item_coeff_manual_edits')
      ) {
        try {
        ...
        key.startsWith('dofus_crushing_ts_') ||
        key.startsWith('dofus_user_item_coeff_timestamps')
      ) {
        try {
          const raw = localStorage.getItem(key);
          if (raw) {
    ...
    if (typeof window !== 'undefined') {
      for (const [key, coeffMap] of Object.entries(data.crushingCoefficients)) {
        const isCoeffKey =
          ...
          key.startsWith('dofus_user_item_coefficients') ||
          key.startsWith('dofus_user_item_coeff_manual_edits');
        if (!isCoeffKey || !coeffMap) continue;
        try {
          if (mode === 'replace') {
