...
export interface HorizontalIngredientCardProps {
  ...
  onSelectBycMethod?: (itemId: number, method: "direct" | "fragments" | "map") => void;
}
...
export const HorizontalIngredientCard: React.FC<HorizontalIngredientCardProps> = ({
  ...
  const bycAnalysis = useMemo(() => {
    if (!isByc) return null;
    return analyzeBycResourceCost(node.item.id, marketPrices);
  }, [isByc, node.item.id, marketPrices]);

  // Si el padre o el almacenamiento tiene un método asignado, se usa ese; sino el mejor costo
  const activeBycMethod: "direct" | "fragments" | "map" =
    selectedBycMethod || (bycAnalysis ? bycAnalysis.bestMethod : "direct");

  const optimalCostForIngredient = isByc
    ? (bycAnalysis
        ? (activeBycMethod === "fragments"
  ...
  const handleApplyBycMethod = (method: "direct" | "fragments" | "map") => {
    if (onSelectBycMethod) {
      onSelectBycMethod(node.item.id, method);
    }
  };
        ...
        {isByc && bycAnalysis && (
          <div className="pt-2 border-t border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-xs">
              ...
              {bycAnalysis.savingsVsDirect > 0 && activeBycMethod !== "direct" && (
            ...
            <div className="grid grid-cols-3 gap-1.5 text-xs">
              <button
                ...
                className={`p-2 rounded-xl border text-center transition flex flex-col items-center justify-center cursor-pointer ${
                  activeBycMethod === "direct"
                    ? "bg-amber-500/20 border-amber-500/60 text-amber-200 font-bold shadow-sm ring-1 ring-amber-500/50"
              ...
              >
                ...
                {bycAnalysis.bestMethod === "direct" && (
                  <span className="text-[9px] text-amber-400 font-normal mt-0.5">Óptimo</span>
                )}
              </button>
              ...
              <button
                ...
                className={`p-2 rounded-xl border text-center transition flex flex-col items-center justify-center cursor-pointer ${
                  activeBycMethod === "fragments"
                    ? "bg-emerald-500/20 border-emerald-500/60 text-emerald-200 font-bold shadow-sm ring-1 ring-emerald-500/50"
                    : "bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-700 hover:text-slate-200"
                }`}
              >
                ...
                {bycAnalysis.bestMethod === "fragments" && (
                  <span className="text-[9px] text-emerald-400 font-normal mt-0.5">Óptimo</span>
                )}
              </button>
